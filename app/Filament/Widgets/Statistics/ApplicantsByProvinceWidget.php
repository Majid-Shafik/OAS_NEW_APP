...
class ApplicantsByProvinceWidget extends BaseWidget
{
    use InteractsWithPageFilters;

    protected static ?string $heading = 'إحصائيات المتقدمين حسب المحافظة';
    ...
    private function getFilterUrl(array $additionalFilters = []): string
    {
        ...
        // Study type filter logic depends on application, typically on applicant it might not exist directly, but we can pass it if we have it
        ...
        return $table
            ...
            ->columns([
                ...
                Tables\Columns\TextColumn::make('total_count')
                    ...
                    ->url(fn($record) => $record->total_count > 0 ? $this->getFilterUrl(['PROVINCE' => ['value' => $record->PROVINCE_NAME]]) : null, shouldOpenInNewTab: true)
                ...
                Tables\Columns\TextColumn::make('updated_count')
                    ...
                    ->url(fn($record) => $record->updated_count > 0 ? $this->getFilterUrl([
                        'PROVINCE' => ['value' => $record->PROVINCE_NAME],
            ...
            ->headerActions([
                ExportAction::make()
                    ->exporter(\App\Filament\Exports\Statistics\ApplicantsByProvinceExporter::class)
                    ->label('تصدير إكسل')
