...
class ApplicantsBySecSchoolTypeWidget extends BaseWidget
{
    use InteractsWithPageFilters;

    protected static ?string $heading = 'إحصائيات المتقدمين حسب نوع الثانوية';
        ...
        return $table
            ->query(function () use ($unid, $faculty, $program, $dateFrom, $dateTo) {
                ...
                return clone \App\Models\ComboValue::query()
                    ...
                    ->withCount([
                        ...
                        'secSchoolApplicants as updated_count' => fn($q) => $applyFilters($q)->where('STATUS', ApplicantStatus::Updated->value),
                        'secSchoolApplicants as ready_count' => fn($q) => $applyFilters($q)->where('STATUS', ApplicantStatus::Ready->value),
                    ])
            ...
            ->columns([
                ...
                Tables\Columns\TextColumn::make('total_count')
                    ...
                    ->url(fn($record) => $record->total_count > 0 ? $this->getFilterUrl(['SEC_SCHOOL_TYPE' => ['value' => (string) $record->VALUE]]) : null, shouldOpenInNewTab: true)
                ...
                Tables\Columns\TextColumn::make('updated_count')
                    ...
                    ->url(fn($record) => $record->updated_count > 0 ? $this->getFilterUrl([
                        'SEC_SCHOOL_TYPE' => ['value' => (string) $record->VALUE],
            ...
            ->headerActions([
               ExportAction::make()
                    ->exporter(\App\Filament\Exports\Statistics\ApplicantsBySecSchoolTypeExporter::class)
                    ->label('تصدير إكسل')
